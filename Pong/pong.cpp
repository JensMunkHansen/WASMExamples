...
EM_JS(void, syncFS, (), { FS.syncfs(function(){}); })
...
struct Ball
{
  ...
  bool isAtTopOrBottom() { return ypos - 5 < 0 || ypos + 5 > 595; }

  bool scoresOnRight() { return xpos > 775; }

  bool scoresOnLeft() { return xpos < 25; }

  bool doesHitPaddle(Paddle left, Paddle right)
  {
    return (xpos - 5 < 50 && left.isAtPaddleLevel(ypos)) ||
      (xpos + 5 > 750 && right.isAtPaddleLevel(ypos));
  }
...
void updatePosition(GameState& gameState)
{
  ...
  if (gameState.ball.doesHitPaddle(gameState.left, gameState.right))
  {
    ...
    gameState.ball.yspeed +=
      calculateReflectionFactor(gameState.ball, gameState.left, gameState.right);
  }
...
{
  auto fpsSurface = TTF_RenderText_Blended(
    font, (std::to_string(fps) + " Frames / Second").c_str(), { 255, 255, 255, 255 });
  ...
  auto timingSurface = TTF_RenderText_Blended(
    font, (std::to_string(averageTiming) + " ms render").c_str(), { 255, 255, 255, 255 });
...
{
  auto leftSurface =
    TTF_RenderText_Blended(font, std::to_string(leftScore).c_str(), { 255, 0, 0, 255 });
  ...
  auto rightSurface =
    TTF_RenderText_Blended(font, std::to_string(rightScore).c_str(), { 0, 0, 255, 255 });
...
EM_BOOL loopIteration(double time, void* gameStateVoidPtr)
{
...
void mainLoop(const std::string& name)
{
  ...
  emscripten_request_animation_frame_loop(loopIteration, &gameState);
}
